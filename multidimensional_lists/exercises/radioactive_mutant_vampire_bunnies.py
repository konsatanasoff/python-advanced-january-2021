def player_dies(player_position, row, col, lair):
    ...
    spread_bunnies(row_count, col_count, lair, player_position)
...
lair = [list(input()) for _ in range(row_count)]
commands = list(input())
...
for cmd in commands:
    ...
    player_position = check_player_move(curr_player_row, curr_player_col, lair, player_position, row_count, col_count)
    spread_bunnies(row_count, col_count, lair, player_position)